...
import {
  FileText, Lock, Unlock, Key, KeyRound, LockKeyhole,
  PenLine, CheckCircle, XCircle, Loader2, BookMarked
} from 'lucide-react';
...
export default function AsymmetricDemo() {
  ...
  const handleEncrypt = () => {
    ...
    try {
      ...
      setEncrypted(forge.util.encode64(enc));
  ...
  const handleSign = () => {
    ...
    try {
      ...
      const b64 = forge.util.encode64(priv.sign(md));
      setSignature(b64);
  ...
  const handleVerify = () => {
    ...
    try {
      ...
      setVerifyResult(pub.verify(md.digest().bytes(), forge.util.decode64(verifySig)));
    } catch {
  ...
  return (
    <div>
      ...
      <div className="flow-diagram">
        ...
        <div className="flow-arrow">→</div>
        <div className="flow-node">
          <KeyRound size={20} strokeWidth={1.5} style={{ margin: '0 auto 4px' }} />
      ...
      <div className="card">
        <div className="card-header">
          <div className="card-icon orange"><LockKeyhole size={18} strokeWidth={2} /></div>
          <div className="card-title">
            <h2>
              Step 1 — Generate RSA <InfoIcon term="rsa" /> Key Pair <InfoIcon term="key_pair" />
            </h2>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
          <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
            Key size <InfoIcon term="key_size" />:
          </span>
          ...
          <span className="badge badge-orange">
            {keySize === 4096 ? 'Slowest' : keySize === 2048 ? 'Recommended' : 'Weak'}
          </span>
        </div>
        ...
        {(publicKey || privateKey) && (
          <>
            ...
            <div className="two-col">
              ...
              <OutputRow label="Private Key (keep secret!)" infoTerm="private_key" value={privateKey} mono={false} />
            </div>
          </>
        )}
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <div className="card-icon red"><PenLine size={18} strokeWidth={2} /></div>
          <div className="card-title">
            <h2>Digital Signatures <InfoIcon term="digital_signature" /> (Sign &amp; Verify)</h2>
            <p>Prove authenticity and integrity using SHA-256 <InfoIcon term="sha256" /> + RSA <InfoIcon term="rsa" /></p>
          </div>
        </div>
        ...
        <div className="two-col">
          ...
          <div>
            <div className="form-group">
              ...
              <textarea value={verifyInput} onChange={e => setVerifyInput(e.target.value)} placeholder="Enter original message…" />
            </div>
            <div className="form-group">
            ...
            {verifyResult !== null && (
              <div style={{ marginTop: 16 }}>
                <div className={`output-box ${verifyResult ? '' : 'error-text'}`} style={{ color: verifyResult ? 'var(--accent2)' : undefined, display: 'flex', alignItems: 'center', gap: 8 }}>
                  {verifyResult
                    ? <><CheckCircle size={16} /> Signature is VALID — message is authentic and unmodified!</>
                    : <><XCircle size={16} /> Signature is INVALID — message was tampered or wrong key used!</>}
                </div>
              </div>
            )}
          </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div className="card-title"><h2>How Asymmetric Encryption Works</h2></div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px,1fr))', gap: 16 }}>
          {[
            { Icon: Key,       title: 'Key Pair',           term: 'key_pair',          desc: 'A mathematically linked pair: public key (shared openly) and private key (kept secret by owner).' },
            { Icon: Lock,      title: 'Encrypt with Public',term: 'public_key',         desc: 'Anyone with your public key can encrypt a message. Only your private key can decrypt it.' },
            { Icon: PenLine,   title: 'Sign with Private',  term: 'digital_signature',  desc: 'You sign data with your private key. Anyone with your public key can verify the signature.' },
            { Icon: Unlock,    title: 'Slower but Safer',   term: null,                 desc: 'Much slower than symmetric encryption. Often used to exchange a symmetric key (hybrid approach).' },
